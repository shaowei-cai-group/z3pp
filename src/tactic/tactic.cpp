...
class unsat_tactic : public tactic {
public:
    void operator()(goal_ref const & in, goal_ref_buffer & result) override {
        in->reset();
        in->assert_expr(in->m().mk_false(), nullptr, nullptr);
        result.push_back(in.get());
    }

    void cleanup() override {}

    char const* name() const override { return "unsat"; }

    tactic * translate(ast_manager & m) override { return this; }
};

tactic * mk_unsat_tactic() {
    return alloc(unsat_tactic);
}
