...
template<bool SYNCH>
std::string mpq_manager<SYNCH>::to_string_bin(mpq const & a, unsigned num_bits) const {
    if (is_int(a))
        return to_string_bin(a.m_num, num_bits);
    return to_string_bin(a.m_num, num_bits) + "/" + to_string_bin(a.m_den, num_bits);
}
