...
public:
    ...
    std::string to_string_bin(mpq const & a, unsigned num_bits) const;

    std::string to_string(mpz const & a) const { return mpz_manager<SYNCH>::to_string(a); }

    std::string to_string_bin(mpz const & a, unsigned num_bits) const { return mpz_manager<SYNCH>::to_string_bin(a, num_bits); }
