...
#define TMP_INITIAL_CAPACITY 128
    ...
    */
    ...
    unsigned manager::sturm(polynomial const * p, var x) {
        ...
        vector<int> lower_sign(vec.size(), 0), upper_sign(vec.size(), 0);
        unsigned change1 = 0, change2 = 0;
        for(unsigned i = 0; i < vec.size(); i++){
            lower_sign[i] = sign_lower_inf(vec[i], x);
            upper_sign[i] = sign_upper_inf(vec[i], x);
            if(i > 0){
                if(lower_sign[i] != lower_sign[i - 1]){
                    change1++;
                }
                if(upper_sign[i] != upper_sign[i - 1]){
                    change2++;
                }
            }
        }
