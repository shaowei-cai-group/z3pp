...
void ls_solver::print_formula_smt(){
    ...
    for(variable &v:_vars){std::cout<<"(declare-fun "<<v.var_name<<" () "<<(v.is_nra?"Int":"Bool")<<")\n";}
    std::cout<<"(assert\n(and\n";
...
void ls_solver::print_literal(lit &l){
    if(!l.is_nra_lit){std::cout<<_vars[l.delta.get_int32()].var_name<<"\n";}
    else{
    for(int i=0;i<l.coff_terms.size();i++){
        std::cout<<"( "<<(l.coff_terms[i].coff).to_string()<<" * ";
        print_term(_terms[l.coff_terms[i].term_idx]);
        std::cout<<" ) + ";
    }
    std::cout<<"( "<<l.key.to_string()<<" ) "<<(l.is_equal?"==":"<=")<<" 0\n";
    }
}

void ls_solver::print_lit_pbs(lit &l){
    rational degree=l.key;
    for(int i=0;i<l.coff_terms.size();i++){degree+=l.coff_terms[i].coff;}
    std::cout<<_num_vars+1<<" "<<(degree)<<" ";
    for(int i=0;i<l.coff_terms.size();i++){std::cout<<(l.coff_terms[i].coff).to_string()<<" "<<-(l.coff_terms[i].term_idx+1)<<" ";}
    std::cout<<"0\n";
}
...
void ls_solver::print_lit_smt(int lit_idx){
    ...
    if(l->is_nra_lit){
        ...
        if(lit_idx>0){std::cout<<_vars[l->delta.get_int32()].var_name<<" ";}
        else{std::cout<<"("<<" not "<<_vars[l->delta.get_int32()].var_name<<" ) ";}
    }
}
...
void ls_solver::print_mv(){
    ...
    for(variable & v:_tmp_vars){
        print_var_solution(v.var_name, value);
        std::string v_name=v.var_name;
        if(v_name.find("#")!=std::string::npos){v_name="|"+v_name+"|";}
        std::cout<<"  (define-fun "<<v_name<<" () Real "<<value<<")\n";
    }
}

void ls_solver::print_mv_vars(uint64_t var_idx){
    variable *v=&(_vars[var_idx]);
    rational var_solution=_solution[var_idx];
    std::cout<<"  (define-fun "<<v->var_name<<" () ";
    if(v->is_nra){
        std::cout<<"Int ";
        if(var_solution>=0){std::cout<<var_solution.to_string()<<")\n";}
        else{std::cout<<"(- "<<(-var_solution).to_string()<<"))\n";}
    }
    else{
        std::cout<<"Bool ";
...
void ls_solver::print_var_solution(std::string &var_name,std::string & var_value){
    ...
    if(name2var.find(var_name)!=name2var.end()){
        ...
        var_value=_solution[var_idx].to_string();
        return;
        ...
        if(name2tmp_var.find(var_name)!=name2tmp_var.end()){
            ...
            var_value=(_solution[new_v_idx]*fa_coff[v_idx]).to_string();
        }//a nra var
        else{
...
void ls_solver::up_bool_vars(){//reconstruct the solution by pop the stack
    for(variable &var:_tmp_vars){
        if(var.is_nra&&name2var.find(var.var_name)==name2var.end()){//if it is an nra var and it is not in the formula
    ...
    for(int lit_idx=0;lit_idx<_lits.size();lit_idx++){
        if(!lit_appear[lit_idx]&&_lits[lit_idx].is_nra_lit){
            lit_appear[lit_idx]=true;
            delta_lit(_lits[lit_idx]);
        }
    }//now all nra lit has delta
    for(int i=0;i<_resolution_vars.size();i++){if(!_resolution_vars[i].is_nra&&_resolution_vars[i].up_bool==0){_resolution_vars[i].up_bool=-1;}}//set all origin bool var as false
        ...
        for(int l_idx:cl.literals){
            ...
            if(l->is_nra_lit){
                if(!lit_appear[std::abs(l_idx)]){
                    rational delta=l->delta;
                    for(coff_term& cf:l->coff_terms){
                        if(!term_appear[cf.term_idx]){sat_flag=true;break;}//if the term does not exist, the nra lit can be true
                    ...
                    //if the nra lit exists
                }
            }
            else{
                if(!lit_appear[std::abs(l_idx)]){
                    if((l_idx>0&&_resolution_vars[l->delta.get_int32()].up_bool>0)||(l_idx<0&&_resolution_vars[l->delta.get_int32()].up_bool<0)){sat_flag=true;break;}}//if the boolean lit does not exist
                else if((_solution[l->delta.get_int32()]>0&&l_idx>0)||(_solution[l->delta.get_int32()]<0&&l_idx<0)){sat_flag=true;break;}
            }
        }
        if(sat_flag==false){
            lit *l=&(_lits[std::abs(cl.literals[0])]);
            _resolution_vars[l->delta.get_int32()].up_bool=1;
