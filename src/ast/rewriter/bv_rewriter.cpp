...
br_status bv_rewriter::mk_extract(unsigned high, unsigned low, expr * arg, expr_ref & result) {
    ...
    // high < sz / 2 is from Bitwuzla zeor_lower_slice
    if (m_util.is_bv_not(arg) ||
        m_util.is_bv_or(arg)  ||
        m_util.is_bv_xor(arg) ||
        (low == 0 && high < sz / 2 && (m_util.is_bv_add(arg) ||
    ...
    // Bitwuzla and_slice
    // `and` has been translated to `not or`
    // match:  (a & b)[upper:lower]
    // result: a[upper:lower] & b[upper:lower]
    // if (m_util.is_bv_and(arg)) {
    //   bool is_simplifiable = false;
    //   ptr_buffer<expr> new_args;
    //   unsigned num = to_app(arg)->get_num_args();
    //   for (unsigned i = 0; i < num; i++) {
    //     expr * curr = to_app(arg)->get_arg(i);
    //     if (is_extract_simplifiable(curr))  {
...
br_status bv_rewriter::mk_bv_udiv_core(expr * arg1, expr * arg2, bool hi_div0, expr_ref & result) {
    ...
    bv_size = m_util.get_bv_size(arg2);

    SASSERT(bv_size == m_util.get_bv_size(arg1));

    // 0 / b --> 0
    if (m_util.is_zero(arg1)) {
      expr *regular = m_util.mk_numeral(0, bv_size);

      expr_ref arg2_eq_zero(m().mk_eq(arg2, mk_numeral(0, bv_size)), m());
      if (hi_div0) {
        result = m().mk_ite(arg2_eq_zero,
            mk_numeral(rational::power_of_two(bv_size) - numeral(1), bv_size),
            regular);
        return BR_REWRITE2;
      }
    ...
    // a / b, where len(a) = 1 --> ~(~a & b)
    if (m_util.get_bv_size(arg1) == 1) {
      if (hi_div0) {
        result = m_util.mk_bv_not(
            m_util.mk_bv_and(m_util.mk_bv_not(arg1), arg2)
            );
        return BR_REWRITE3;
      }
      else {
        // TBD: not worthy for `!hi_div0` case
      }
    }

    // power2_udiv has already be done

    // a / a
    if (arg1 == arg2) {
      expr_ref  arg2_eq_zero (m().mk_eq(arg2, mk_numeral(0, bv_size)), m());
      expr *regular = m_util.mk_numeral(1, bv_size);
    ...
    // result: x ? a / c : b / d
    expr *x = nullptr, *a = nullptr, *b = nullptr;
    expr *y = nullptr, *c = nullptr, *d = nullptr;
    if (m().is_ite(arg1, x, a, b) &&
        m().is_ite(arg2, y, c, d)) {
      if (x == y && (a == c || b == d)) {
        result = m().mk_ite(x,
            m_util.mk_bv_udiv(a, c),
            m_util.mk_bv_udiv(b, d));
        return BR_REWRITE2;
      }
    }
...
br_status bv_rewriter::mk_bv_urem_core(expr * arg1, expr * arg2, bool hi_div0, expr_ref & result) {
    ...
    if (!hi_div0) {
        ...
        // bool, a % b --> a & ~b
        if (get_bv_size(arg1) == 1) {
          result = m().mk_ite(
              m().mk_eq(arg2, mk_numeral(0, 1)),
              m_util.mk_bv_urem0(arg1),
              m_util.mk_bv_and(arg1, m_util.mk_bv_not(arg2)));
          return BR_REWRITE3;
        }
        ...
        if (arg1 == arg2) {
          result = m().mk_ite(
              m().mk_eq(arg2, mk_numeral(0, get_bv_size(arg2))),
              m_util.mk_bv_urem0(arg1),
              mk_numeral(numeral(0), get_bv_size(arg1)));
          return BR_REWRITE2;
        }
    }
    else {
        ...
        // bool, a % b --> a & ~b
        if (get_bv_size(arg1) == 1) {
          result = m_util.mk_bv_and(arg1, m_util.mk_bv_not(arg2));
          return BR_REWRITE2;
        }

        // urem(a, a) --> 0
        if (arg1 == arg2) {
          result = mk_numeral(numeral(0), get_bv_size(arg1));
          return BR_DONE;
        }
    }
...
br_status bv_rewriter::mk_concat(unsigned num_args, expr * const * args, expr_ref & result) {
    ...
    for (unsigned i = 0; i < num_args; i++) {
        ...
        // [Lin] TODO need to propagte forward or backward
        // we also need to decide the rewrite level combine with extract
        // else if (prev != nullptr && m_util.is_bv_not(arg) &&
        //     m_util.is_bv_or(to_app(arg)->get_arg(0))) {
        //   // Bitwuzla
        //   // match: c::(a & b)
        //   // result: (c::a) & (c::b)
        //   // a & b <=> - (-a | -b)
        //   // match: c::-(-a | -b)
        //   // result: -(-c::-a | -c::-a)
        //   expr *not_a = to_app(to_app(arg)->get_arg(0))->get_arg(0);
        //   expr *not_b = to_app(to_app(arg)->get_arg(0))->get_arg(1);
        //   expr *not_c = m_util.mk_bv_not(prev);
        //   expr *new_arg = m_util.mk_bv_not(m_util.mk_bv_or(
        //         m_util.mk_concat(not_c, not_a),
        //         m_util.mk_concat(not_c, not_b)));
        //   new_args.pop_back();
        //   new_args.push_back(new_arg);
        //   fused_and = true;
        // }
        // else if (prev != nullptr && m_util.is_bv_not(prev) &&
        //     m_util.is_bv_or(to_app(prev)->get_arg(0))) {
        //   // Bitwuzla
        //   // match: (a & b)::c
        //   // result: (a::c) & (b::c)
        //   // a & b <=> - (-a | -b)
        //   // match: -(-a | -b)::c
        //   // result: -(-a::-c | -b::-c)
        //   expr *not_a = to_app(to_app(prev)->get_arg(0))->get_arg(0); 
        //   expr *not_b = to_app(to_app(prev)->get_arg(0))->get_arg(1); 
        //   expr *not_c = m_util.mk_bv_not(arg);
        //   expr *new_arg = m_util.mk_bv_not(m_util.mk_bv_or(
        //         m_util.mk_concat(not_a, not_c),
        //         m_util.mk_concat(not_b, not_c)));
        //
        //   new_args.pop_back();
        //   new_args.push_back(new_arg);
        //   fused_and = true;
...
bool bv_rewriter::eq_not(expr *left, expr *right) {
    if ((m_util.is_bv_not(left) && to_app(left)->get_arg(0) == right) ||
        (m_util.is_bv_not(right) && to_app(right)->get_arg(0) == left)) {
      return true;
    }
    numeral lval, rval;
    if (m_util.is_numeral(left, lval) && m_util.is_numeral(right, rval) &&
        lval == bitwise_not(get_bv_size(right), rval)) {
      return true;
    }
    return false;
}

// ~(a or b) or ~(c or d) i.e. (~a & ~b) or (~c & ~d)
bool bv_rewriter::resolution(expr_ref_buffer &args, unsigned int i, unsigned int j, expr_fast_mark2 &is_res) {
  expr *left = args[i];
  expr *right = args[j];
  if (!m_util.is_bv_not(left) || !m_util.is_bv_not(right)) {
    return false;
  }
  if (!m_util.is_bv_or(to_app(left)->get_arg(0)) ||
      !m_util.is_bv_or(to_app(right)->get_arg(0)) ||
      to_app(to_app(left)->get_arg(0))->get_num_args() != 2 ||
      to_app(to_app(right)->get_arg(0))->get_num_args() != 2) {
    return false;
  }
  expr *not_left = to_app(left)->get_arg(0);
  expr *not_right = to_app(right)->get_arg(0);
  expr *a = to_app(not_left)->get_arg(0);
  expr *b = to_app(not_left)->get_arg(1);
  expr *c = to_app(not_right)->get_arg(0);
  expr *d = to_app(not_right)->get_arg(1);

  // if a = c && b = ~d or a = d && b = ~c
  // result = ~a
  if ((a == c && eq_not(b, d)) ||
      (a == d && eq_not(b, c))) {
    if (m_util.is_bv_not(a)) {
      args.set(i, to_app(a)->get_arg(0));
    }
    else {
      args.set(i, m_util.mk_bv_not(a));
    }
    is_res.mark(args[i]);
    if (j != args.size() - 1) { // remove args[j]
  ...
  // if b = c && a = ~d or b = d & a = ~c
  // result = ~b
  if ((b == c && eq_not(a, d)) ||
      (b == d && eq_not(a, c))) {
    if (m_util.is_bv_not(b)) {
      args.set(i, to_app(b)->get_arg(0));
    }
    else {
      args.set(i, m_util.mk_bv_not(b));
    }
    is_res.mark(args[i]);
    if (j != args.size() - 1) { // remove args[j]
      args.set(j, args[args.size() - 1]);
...
// (a or ~c) and (a or ~d)
// if a = c => a or ~d
bool bv_rewriter::subst_bv_or(expr_ref_buffer &args, unsigned int i, unsigned int j, expr_fast_mark2 &is_res) {
  expr *left = args[i];
  expr *right = args[j];

  bool swapped = false;
SWAP:
  if (m_util.is_bv_not(right) &&
      m_util.is_bv_or(to_app(right)->get_arg(0)) &&
      to_app(to_app(right)->get_arg(0))->get_num_args() == 2) {
    expr *c = to_app(to_app(right)->get_arg(0))->get_arg(0);
    expr *d = to_app(to_app(right)->get_arg(0))->get_arg(1);
    if (left != c) {
      std::swap(c, d);
    }
    if (left == c) {
      if (m_util.is_bv_not(d)) {
        args.set(i, to_app(d)->get_arg(0));
      }
      else {
        args.set(i, m_util.mk_bv_not(d));
      }
      args.set(j, left);
      is_res.mark(args[i]);
      return true;
...
// a or ~(c or d) -> a or (~c and ~d)
// (a or ~c) and (a or ~d)
// if a = ~c => a and (a or ~d) => a
// if a = ~d => (a or ~c) and a => a
bool bv_rewriter::subsum_bv_or(expr_ref_buffer &args, unsigned int i, unsigned int j, expr_fast_mark2 &is_res) {
  expr *left = args[i];
  expr *right = args[j];
  bool is_swap = false;
SWAP_OPERANDS:
...
bool bv_rewriter::square_apply(expr_ref_buffer &args, Proc proc) {
    bool proced = false;
    unsigned int apply_num = args.size(); // number of new generate exp
    while (true) {
      expr_fast_mark2 is_res;
      // unsigned num_args = args.size();
      unsigned int new_apply_num = 0;
      for (unsigned i = 0; i < apply_num && i < args.size(); ++i) {
        expr_ref res(m());
        for (unsigned j = i + 1; j < args.size(); ++j) {
          if ((this->*proc)(args, i, j, is_res)) {
            ++new_apply_num;
            break;
          }
        }
      ...
      proced = true;
      apply_num = new_apply_num;

      // move generated expr infront
      unsigned int fix_num = args.size();
      for (unsigned i = 0, j = 0; j < apply_num && i < fix_num; ++i) {
        if (is_res.is_marked(args[i])) { // swap args[i] and args[j]
          if (i != j) {
            expr_ref tmp_arg(args[i], m());
            args.set(i, args[j]);
            args.set(j, tmp_arg);
          }
          ++j;
...
br_status bv_rewriter::mk_bv_or(unsigned num, expr * const * args, expr_ref & result) {
    ...
    for (unsigned i = 0; i < num; i++) {
        expr * arg = args[i];
        if (is_numeral(arg, v2, sz)) { // [Lin] bitwise_or all constants
            num_coeffs++;
            v1 = bitwise_or(v1, v2);
            continue;
        }

        // [Lin] a or a --> a; a or ~a --> true
        if (m_util.is_bv_not(arg)) {
    ...
    // Bitwuzla
    // ~(a or b) or ~(c or d) i.e. (~a & ~b) or (~c & ~d) -> ~a
    // if a = c && b = ~d or a = d && b = ~c
    bool new_app = false;
    bool elim = false;
    expr_ref_buffer tmp_args(m());
    for (unsigned i = 0; i < new_args.size(); ++i) {
      tmp_args.push_back(new_args[i]);
    }
    if (square_apply(tmp_args, &bv_rewriter::resolution)) {
      new_app = true;
    }
    ...
    // result: a & b
    // a or (~c and ~d) -> (a or ~c) and (a or ~d)
    // => a or ~d, if a = c
    if (square_apply(tmp_args, &bv_rewriter::subst_bv_or)) {
      new_app = true;
    }
    ...
    // if a = ~c => a and (a or ~d) => a
    // if a = ~d => (a or ~c) and a => a
    if (square_apply(tmp_args, &bv_rewriter::subsum_bv_or)) {
      elim = true;
    }
    ...
    if (new_app || elim) {
      SASSERT(new_args.size() >= tmp_args.size());
      for (unsigned i = 0; i < tmp_args.size(); ++i) {
        new_args.set(i, tmp_args[i]);
      }
      new_args.shrink(tmp_args.size());
    }

    // Bitwuzla end


    if (!flattened && !merged && !new_app && !elim && (num_coeffs == 0 || (num_coeffs == 1 && !v1.is_zero())) && (!m_bv_sort_ac || is_sorted(num, args))) {
        return BR_FAILED;
    }
...
br_status bv_rewriter::mk_bool_app(func_decl* f, unsigned n, expr* const* args, expr_ref& result) {
    switch (f->get_decl_kind()) {
    case OP_AND:
        return mk_bool_and(n, args, result);
    case OP_OR:
        return mk_bool_or(n, args, result);
    default:
        return BR_FAILED;
    }
}

// Note all rel op has been rewriten to ule and sle
// TODO: top-level `and' are in different index of goal
// m_elim_and will change `and' to `or'
br_status bv_rewriter::mk_bool_and(unsigned num, expr * const *args, expr_ref &result) {
  ptr_buffer<expr> rel_args;
  ptr_buffer<expr> rel_not_args;
  ptr_buffer<expr> srel_args;
  ptr_buffer<expr> srel_not_args;

  for (unsigned i = 0; i < num; ++i) {
    if (m_util.is_bv_ule(args[i])) {
      rel_args.push_back(args[i]);
    }
    else if (m_util.is_bv_sle(args[i])) {
      srel_args.push_back(args[i]);
    }
    else if (m().is_not(args[i]) && m_util.is_bv_ule(to_app(args[i])->get_arg(0))) {
      rel_not_args.push_back(args[i]);
    }
    else if (m().is_not(args[i]) && m_util.is_bv_sle(to_app(args[i])->get_arg(0))) {
      srel_not_args.push_back(args[i]);
    }
  }

  // ~(a <= b) & ~(b <= a) -> false
  for (unsigned i = 0; i < rel_not_args.size(); ++i) {
    expr *a = to_app(to_app(rel_not_args[i])->get_arg(0))->get_arg(0);
    expr *b = to_app(to_app(rel_not_args[i])->get_arg(0))->get_arg(1);
    for (unsigned j = i + 1; j < rel_not_args.size(); ++j) {
      if (a == to_app(to_app(rel_not_args[j])->get_arg(0))->get_arg(1) &&
          b == to_app(to_app(rel_not_args[j])->get_arg(0))->get_arg(0)) {
        result = m().mk_false();
        return BR_DONE;
      }
    }
  }
  for (unsigned i = 0; i < srel_not_args.size(); ++i) {
    expr *a = to_app(to_app(srel_not_args[i])->get_arg(0))->get_arg(0);
    expr *b = to_app(to_app(srel_not_args[i])->get_arg(0))->get_arg(1);
    for (unsigned j = i + 1; j < srel_not_args.size(); ++j) {
      if (a == to_app(to_app(srel_not_args[j])->get_arg(0))->get_arg(1) &&
          b == to_app(to_app(srel_not_args[j])->get_arg(0))->get_arg(0)) {
        result = m().mk_false();
        return BR_DONE;
      }
    }
  ...
  // (a <= b) & (b <= a) -> a = b
  bool is_rewrited =  false;
  expr_ref_buffer new_args(m());
  expr_fast_mark1 is_apply;
  for (unsigned i = 0; i < rel_args.size(); ++i) {
    if (is_apply.is_marked(rel_args[i])) {
      continue;
    }
    expr *a = to_app(rel_args[i])->get_arg(0);
    expr *b = to_app(rel_args[i])->get_arg(1);
    for (unsigned j = i + 1; j < rel_args.size(); ++j) {
      if (is_apply.is_marked(rel_args[j])) {
        continue;
      }
      if (a == to_app(rel_args[j])->get_arg(1) &&
          b == to_app(rel_args[j])->get_arg(0)) {
        is_apply.mark(rel_args[i]);
        is_apply.mark(rel_args[j]);
        new_args.push_back(m().mk_eq(a, b));
        is_rewrited = true;
        break;
      }
    }
  ...
  // signed
  for (unsigned i = 0; i < srel_args.size(); ++i) {
    if (is_apply.is_marked(srel_args[i])) {
      continue;
    }
    expr *a = to_app(srel_args[i])->get_arg(0);
    expr *b = to_app(srel_args[i])->get_arg(1);
    for (unsigned j = i + 1; j < srel_args.size(); ++j) {
      if (is_apply.is_marked(srel_args[j])) {
        continue;
      }
      if (a == to_app(srel_args[j])->get_arg(1) &&
          b == to_app(srel_args[j])->get_arg(0)) {
        is_apply.mark(srel_args[i]);
        is_apply.mark(srel_args[j]);
        new_args.push_back(m().mk_eq(a, b));
        is_rewrited = true;
        break;
      }
...
br_status bv_rewriter::mk_bool_or(unsigned num, expr * const *args, expr_ref &result) {
  ptr_buffer<expr> rel_args;
  ptr_buffer<expr> rel_not_args;
  ptr_buffer<expr> srel_args;
  ptr_buffer<expr> srel_not_args;
  ...
  // ~(a <= b) or ~(b <= a) -> ~(a = b)
  unsigned char is_rewrited = 0;
  expr_ref_buffer new_args(m());
  expr_fast_mark1 is_apply;
  for (unsigned i = 0; i < rel_not_args.size(); ++i) {
    if (is_apply.is_marked(rel_not_args[i])) {
      continue;
    }
    expr *a = to_app(to_app(rel_not_args[i])->get_arg(0))->get_arg(0);
    expr *b = to_app(to_app(rel_not_args[i])->get_arg(0))->get_arg(1);
    for (unsigned j = i + 1; j < rel_not_args.size(); ++j) {
      if (is_apply.is_marked(rel_not_args[j])) {
        continue;
      }
      if (a == to_app(to_app(rel_not_args[j])->get_arg(0))->get_arg(1) &&
          b == to_app(to_app(rel_not_args[j])->get_arg(0))->get_arg(0)) {
        is_apply.mark(rel_not_args[i]);
        is_apply.mark(rel_not_args[j]);
        new_args.push_back(m().mk_not(m().mk_eq(a, b)));
        is_rewrited |= (1 << 1);
        break;
      }
  ...
  // signed
  for (unsigned i = 0; i < srel_not_args.size(); ++i) {
    if (is_apply.is_marked(srel_not_args[i])) {
      continue;
    }
    expr *a = to_app(to_app(srel_not_args[i])->get_arg(0))->get_arg(0);
    expr *b = to_app(to_app(srel_not_args[i])->get_arg(0))->get_arg(1);
    for (unsigned j = i + 1; j < srel_not_args.size(); ++j) {
      if (is_apply.is_marked(srel_not_args[j])) {
        continue;
      }
      if (a == to_app(to_app(srel_not_args[j])->get_arg(0))->get_arg(1) &&
          b == to_app(to_app(srel_not_args[j])->get_arg(0))->get_arg(0)) {
        is_apply.mark(srel_not_args[i]);
        is_apply.mark(srel_not_args[j]);
        new_args.push_back(m().mk_not(m().mk_eq(a, b)));
        is_rewrited |= (1 << 1);
        break;
      }
...
br_status bv_rewriter::mk_bv_add(unsigned num_args, expr * const * args, expr_ref & result) {
    ...
    // [Lin] convert to BVOR if for each bit, at most one args has non-zore bit
    struct out {};
    try {
      for (unsigned i = 0; i < sz; i++) {
    ...
    // bool_add --> xor
    if (sz == 1) {
      result = m_util.mk_bv_xor(_num_args, _args);
      return BR_REWRITE1;
    }

    // not_add alraedy done

    // bcond_add
    // TODO move to pull_cheap_ite
    if (mk_add_pull_ite(_num_args, _args, result)) {
      return BR_REWRITE2;
    }

    // BUG? it has been done by setting hosit_mul
    // if enable, many instance of sage benchmark can't be solved
    // if (mk_add_hoist_mul(_num_args, _args,  result)) {
    //   return BR_REWRITE2;
    // }
...
SWAP_OPERANDS:
    ...
    try {
      ...
      numeral k;
      if (m_util.is_bv_neg(x)) {
        mul = to_app(x)->get_arg(0);
      }
      else if (m_util.is_bv_mul(x) && to_app(x)->get_num_args() == 2 && is_minus_one(to_app(x)->get_arg(0))) {
        mul = to_app(x)->get_arg(1);
      }
      else {
        throw out();
      }

      if (m_util.is_bv_mul(mul) && to_app(mul)->get_num_args() == 2) {
        if (m_util.is_bv_udiv(to_app(mul)->get_arg(0)) ||
            m_util.is_bv_udivi(to_app(mul)->get_arg(0))) {
          udiv = to_app(mul)->get_arg(0);
          b = to_app(mul)->get_arg(1);
        }
        else if (m_util.is_bv_udiv(to_app(mul)->get_arg(1)) ||
            m_util.is_bv_udivi(to_app(mul)->get_arg(1))) {
          udiv = to_app(mul)->get_arg(1);
          b = to_app(mul)->get_arg(0);
        }
        else {
          throw out();
        }
    ...
    // match:  a + (a * b)
    // result: (a * (b + 1))
    // done in mk_add_hoist_mul
    // if (!m_som && m_util.is_bv_mul(y) && (x == to_app(y)->get_arg(0))) {
    //   expr *b = to_app(y)->get_arg(1);
    //   result = m_util.mk_bv_mul(x, 
    //       m_util.mk_bv_add(mk_numeral(1, get_bv_size(b)), b));
    //   return BR_REWRITE2;
    // }
 ...
 * result: (a * (b + 1))
 */
bool bv_rewriter::mk_add_hoist_mul(unsigned num_args, expr * const * args, expr_ref & result) {
  // TODO for num_args > 2 case 
  if (num_args != 2) {
    return false;
  }
  if (m_som) {
  ...
  expr *x = args[0], *y = args[1];
    ...
    unsigned k = m1.size();
    expr_ref_buffer new_args(m());
    new_args.push_back(m_util.mk_numeral(numeral(1), get_bv_size(x)));
    new_args.push_back(mk_mul_app(m2.size()-k, m2.data()+k));
    m1.push_back(mk_add_app(new_args.size(), new_args.data()));
    result = mk_mul_app(k+1, m1.data());
...
// match:  (x ? a : b) + (x ? c : d), where either a = c or b = d
// result: x ? (a + c) : (b + d)
bool bv_rewriter::mk_add_pull_ite(unsigned num_args, expr * const * args, expr_ref & result) {
#if 0
  // TODO how to check a = c or b = d
  SASSERT(num_args >= 2);
  expr *x = nullptr, *a = nullptr, *b = nullptr;
  expr_fast_mark1 visited;  // visited.is_marked(x) if the ite(x, a, b) occurs in args
  expr_fast_mark2 cond; // multiple.is_marked(x) if ite(x, ..., ...) occurs more than once
  bool has_cond = false;

  for (unsigned i = 0; i < num_args; i++) {
    expr * arg = args[i];

    if (m().is_ite(arg, x, a, b)) {
  ...
  TRACE("rewriter", 
      tout << "mk_add_pull_ite " << "\n";
      for (unsigned i = 0; i < num_args; i++) tout << mk_ismt2_pp(args[i], m()) << "\n";);

  if (has_cond) {
    // expensive case
    expr_ref_vector new_args(m());
    vector<expr_ref_vector> vec_c_args;
    m_expr2pos.reset();
    // group ite by condition
    for (unsigned i = 0; i < num_args; i++) {
      expr * arg = args[i];
      if (!m().is_ite(arg, x, a, b) || !cond.is_marked(x)) {
        new_args.push_back(arg);
        continue;
      }
      ...
      unsigned pos;
      if (m_expr2pos.find(x, pos)) {
        vec_c_args[pos].push_back(arg);
      }
      else {
        m_expr2pos.insert(x, vec_c_args.size());
        vec_c_args.push_back(expr_ref_vector(m()));
        vec_c_args.back().push_back(arg);
      }
    }

    // copy power products with non zero coefficients to new_args
    unsigned vec_c_sz = vec_c_args.size();
    for (unsigned i = 0; i < vec_c_sz; i++) {
      expr_ref_buffer left_args(m()), right_args(m());
      unsigned num_c_args = vec_c_args[i].size();
      for (unsigned j = 0; j < num_c_args; ++j) {
        SASSERT(m().is_ite(vec_c_args[i][j].get()));
        m().is_ite(vec_c_args[i][j].get(), x, a, b);
        left_args.push_back(a);
        right_args.push_back(b);
      }
      expr_ref  left_res = mk_add_app(left_args.size(), left_args.data());
      expr_ref  right_res = mk_add_app(right_args.size(), right_args.data());

      new_args.push_back(m().mk_ite(x, left_res, right_res));
    }
  ...
  expr *x = nullptr, *a = nullptr, *b = nullptr;
  expr *y = nullptr, *c = nullptr, *d = nullptr;
  if (m().is_ite(args[0], x, a, b) && m().is_ite(args[1], y, c, d) && 
      x == y && (a == c || b == d) ) {
    result = m().mk_ite(x, 
        m_util.mk_bv_add(a, c),
        m_util.mk_bv_add(b, d));
    return true;
  }
  return false;
...
br_status bv_rewriter::mk_bv_mul(unsigned num_args, expr * const * args, expr_ref & result) {
    ...
    if (m_mul2concat) { // [Lin] number is before others after mk_mul_core
        numeral v;
        unsigned bv_size;
        unsigned shift;
    ...
    // TODO: check recursive BR_REWRITE
    // TODO: both of arg1 and arg2 is bv_not
    // --------------------------------------------
    // [Lin] Bitwuzla
    // --------------------------------------------

    // this `if-then-else` code can be removed
    if (st == BR_FAILED && num_args == 2) {
        x = args[0]; y = args[1];
    }
    else if (st == BR_DONE && is_mul(result) && to_app(result)->get_num_args() == 2) {
    ...
    // Already done in poly_rewriter: const_binary, special_const (0, 1)
    // const_lhs_mul, const_rhs_mul, const_mul
    
    // special const: arg is 111...111
    // This rule conflict with mk_uminus
    // SASSERT(is_numeral(x) || !is_numeral(y)); // number is before others after mk_mul_core
    // if (m_util.is_allone(x)) {
    //   result = m_util.mk_bv_neg(y);
    //   return BR_REWRITE1;
    // }

    // if sz = 1,  x * y --> x and y
    if (m_util.get_bv_size(x) == 1) {
      SASSERT(m_util.get_bv_size(y) == 1);
      result = m_util.mk_bv_and(x, y);
      return BR_REWRITE1;
    }
...
SWAP_OPERANDS:
    ...
    // (bvmul u (ite a b c)) --> (ite a (bvmul u b) (bvmul u c)) if b == 0 
    // bad for some yices instances
    // TODO: move to pull_cheap_ite?
    expr* a = nullptr, *b = nullptr, *c = nullptr;
    if (m().is_ite(y, a, b, c) && (m_util.is_zero(b) || m_util.is_zero(c))) {
      expr_ref mul_left(m_util.mk_bv_mul(x, b), m());
      expr_ref mul_right(m_util.mk_bv_mul(x, c), m());
      result = m().mk_ite(a, mul_left, mul_right);
      return BR_REWRITE2;
    }
...
br_status bv_rewriter::mk_bit2bool(expr * lhs, expr * rhs, expr_ref & result) {
    ...
    if (is_numeral(lhs)) {
        ...
        result = m().mk_bool_val(lhs == rhs); // [Lin] 指针相等就是值相等
        return BR_DONE;
    }

    // [Lin] (ite a b c) = const -->
    // (ite a (b = const) (c = const))
    expr* a = nullptr, *b = nullptr, *c = nullptr;
...
bool bv_rewriter::is_always_unequal(expr *lhs, expr *rhs) {
  ...
  // c + a != b, if c != 0, a = b
  // after rewrite bv add, numeral is at the begining
  ...
  expr *c1 = nullptr, *a1 = nullptr;
  expr *tmp_rhs = rhs;
  is_not = false;
  if (m_util.is_bv_not(rhs, tmp_rhs)) {
    is_not = true;
  }
  if (m_util.is_bv_add(tmp_rhs, c1, a1) &&
      m_util.is_numeral(c1, val) &&
      !val.is_zero()) {
  ...
  // if a0 = a1 && c0 != c1, then a0 + c0 != a1 + c1
  if ((m_util.is_bv_not(lhs) == m_util.is_bv_not(rhs)) && c0 && c1 && is_numeral(c0) && is_numeral(c1)) {
    if (a0 == a1 && c0 != c1) {
      return true;
    }
  }
...
bool bv_rewriter::norm_eq(expr *&lhs, expr *&rhs) {
    bool do_normalize = false;
    // ~x = ~y -> x = y
    if (m_util.is_bv_not(lhs) && m_util.is_bv_not(rhs)) {
    ...
    numeral val;
    unsigned bv_size;
    if (m_util.is_numeral(rhs, val, bv_size) && m_util.is_bv_not(lhs)) {
      lhs = to_app(lhs)->get_arg(0);
      val = bitwise_not(bv_size, val);
      rhs = mk_numeral(val, bv_size);
      do_normalize = true;
    }
...
br_status bv_rewriter::mk_eq_core(expr * lhs, expr * rhs, expr_ref & result) {
    ...
    br_status st;
    // [Lin] eq for 1bit operand
    // TODO disable for (= (bvxor a b) 1), as it can be rewrited as (~a = b)
    if (m_bit2bool) {
    ...
    if (m().is_ite(lhs) && m().is_ite(rhs) &&
        to_app(lhs)->get_arg(0) == to_app(rhs)->get_arg(0) &&
        (to_app(lhs)->get_arg(1) == to_app(rhs)->get_arg(1) ||
        to_app(lhs)->get_arg(2) == to_app(rhs)->get_arg(2))) {
      result = m().mk_ite(to_app(lhs)->get_arg(0),
          m().mk_eq(to_app(lhs)->get_arg(1), to_app(rhs)->get_arg(1)),
          m().mk_eq(to_app(lhs)->get_arg(2), to_app(rhs)->get_arg(2)));
          return BR_REWRITE2;
    }
    ...
    numeral val;
    unsigned bv_size;
    if (m_util.is_bv_xor(lhs) && is_numeral(rhs, val) && val.is_zero()) {
      unsigned sz = to_app(lhs)->get_num_args();
      expr * t1 = to_app(lhs)->get_arg(0);
      expr_ref t2(m());
      if (sz > 2) {
        t2 = m().mk_app(get_fid(), OP_BXOR, sz-1, to_app(lhs)->get_args()+1);
        result = m().mk_eq(t1, t2);
        return BR_REWRITE2;
      }
      else {
        SASSERT(sz == 2);
        t2 = to_app(lhs)->get_arg(1);
        result = m().mk_eq(t1, t2);
        return BR_REWRITE1;
      }
    }
    ...
    // a xor b = ones -> ~a = b
    if (m_util.is_bv_xor(lhs) && is_numeral(rhs, val, bv_size) && val == rational::power_of_two(bv_size) - numeral(1)) {
      unsigned sz = to_app(lhs)->get_num_args();
      expr * t1 = to_app(lhs)->get_arg(0);
      expr_ref t2(m());
      if (sz > 2) {
      ...
      result = m().mk_eq(m_util.mk_bv_not(t1), t2);
      return BR_REWRITE2;
    }

    // a or b = zero -> a = 0 & b = 0
    if (m_util.is_bv_or(lhs) && is_numeral(rhs, val, bv_size) && val.is_zero()) {
      expr_ref_buffer new_args(m());
      unsigned num_args = to_app(lhs)->get_num_args();
      for (unsigned i = 0; i < num_args; ++i) {
        new_args.push_back(m().mk_eq(to_app(lhs)->get_arg(i), rhs)); // rhs is zero
      }
    ...
    // similar to mk_blast_eq_value
    bv_size = get_bv_size(lhs);
    if (bv_size > 1 && is_numeral(rhs, val, bv_size) &&
        !val.is_zero() && val != rational::power_of_two(bv_size) - numeral(1) &&
        (m_util.is_bv_or(lhs) || m_util.is_bv_xor(lhs))) {
      char tmpstr[2] = {'\0', '\0'};
      unsigned pos = 0;
      std::string str = val.to_string_bin(bv_size);
      const char *bvstr = str.c_str();
      // char bvstr[bv_size];
      // unsigned leading_sz = bv_size - str.size();
      // for (unsigned i = 0; i < leading_sz; ++i) {
      //   bvstr[i] = '0';
      // }
      // for (unsigned i = leading_sz, j = 0; i < bv_size; ++i, ++j) {
      //   bvstr[i] = str[j];
      // }
      // bvstr[bv_size] = '\0';
      ...
      while (pos < bv_size) {
        tmpstr[0] = bvstr[pos];
        unsigned len = strspn(bvstr + pos, tmpstr);
        expr *tmp = m_util.mk_extract(bv_size - 1 - pos, bv_size - pos - len, lhs);

        expr *num;
        if (tmpstr[0] == '0') {
          num = m_util.mk_numeral(0, len);
        }
        else {
          SASSERT(tmpstr[0] == '1');
          num = m_util.mk_numeral(rational::power_of_two(len) - rational::one(), len);
        }
        new_args.push_back(m().mk_eq(tmp, num));
        pos += len;
      }
      result = m().mk_and(new_args.size(), new_args.data());
      return BR_REWRITE3;
...
SWAP_OPERANDS:
    expr *b = nullptr, *t = nullptr, *e = nullptr;
    // TODO it is bad for bin_libsmbsharemodes instances
    // if (m().is_ite(lhs, b, t, e)) {
    //   // match:  (b ? t : e) = t
    //   // result: b OR e = t
    //   if (t == rhs) {
    //     result = m().mk_or(b,
    ...
    if (m().is_ite(lhs, b, t, e)) {
      // match:  (b ? t : e) = rhs, where t != rhs
      // result: !b AND e = rhs
      expr *tmp_t = t, *tmp_rhs = rhs;
      bool uneq = false;
      if (is_numeral(tmp_t) && is_numeral(tmp_rhs) && tmp_t != tmp_rhs) {
        uneq = true;
      }
      norm_eq(tmp_t, tmp_rhs);
      if (is_always_unequal(tmp_t, tmp_rhs)) {
        uneq = true;
      }
      if (uneq) {
        result = m().mk_and(m().mk_not(b),
            m().mk_eq(e, rhs));
        return BR_REWRITE2;
      }
      // match:  (b ? t : e) = rhs, where e != rhs
      // result: b AND t = rhs
      expr *tmp_e = e;
      tmp_rhs = rhs;
      uneq = false;
      if (is_numeral(tmp_e) && is_numeral(tmp_rhs) && tmp_e != tmp_rhs) {
        uneq = true;
      }
      norm_eq(tmp_e, tmp_rhs);
      if (is_always_unequal(tmp_e, tmp_rhs)) {
        uneq = true;
      }
      if (uneq) {
        result = m().mk_and(b,
    ...
    if (m_util.is_bv_not(lhs) &&
        m().is_ite(to_app(lhs)->get_arg(0), b, t, e)) {
      // match:  ~(b ? t : e) = t
      // result: !b AND ~e = t
      if (t == rhs) {
        result = m().mk_and(m().mk_not(b),
            m().mk_eq(m_util.mk_bv_not(e), t));
        return BR_REWRITE3;
      }
      // match:  ~(b ? t : e) = e
      // result: b AND ~t = e
      if (e == rhs) {
        result = m().mk_and(b,
            m().mk_eq(m_util.mk_bv_not(t), e));
        return BR_REWRITE3;
      }
    }
  

    // TODO remove? it has been done in the second round of simplifies_tactic
    // ADD_RW_RULE(distrib_add_mul_eq, e0, e1);
    // match:  a * (b + c) = a * b + a * c
    // result: true
    if (distrib_add_mul_eq(lhs, rhs, result) == BR_DONE) {
      SASSERT(result = m().mk_true());
      return BR_DONE;
    }
    ...
    std::swap(lhs, rhs); // swap back if no rewriting
