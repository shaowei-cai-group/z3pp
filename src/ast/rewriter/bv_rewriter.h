...
class bv_rewriter : public poly_rewriter<bv_rewriter_core> {
    ...
    using Proc = bool (bv_rewriter::*) (expr_ref_buffer &args, unsigned int i, unsigned int j, expr_fast_mark2 &is_res);
    bool eq_not(expr *left, expr *right);
    ...
    br_status mk_bv_or_old(unsigned num, expr * const * args, expr_ref & result);
    ...
    bool mk_add_hoist_mul(unsigned num_args, expr * const * args, expr_ref & result);

    bool mk_add_pull_ite(unsigned num_args, expr * const * args, expr_ref & result);

    // [Lin] TODO check whether the usage of is_uninterp_const is correct
    bool is_extract_simplifiable(expr const *e) { return m_util.is_numeral(e) || m_util.is_extract(e) || is_uninterp_const(e); }
...
public:
    ...
    bool is_numeral(expr *t, numeral &r) const { unsigned sz; return m_util.is_numeral(t, r, sz); }
    ...
    br_status mk_bool_and(unsigned num, expr * const *args, expr_ref &result);
    br_status mk_bool_or(unsigned num, expr * const *args, expr_ref &result);
