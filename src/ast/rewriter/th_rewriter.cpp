    ...
    br_status reduce_app_core(func_decl * f, unsigned num, expr * const * args, expr_ref & result) {
        ...
        if (fid == m_b_rw.get_fid()) {
            ...
            if (k == OP_AND || k == OP_OR) {
              ...
              if (is_app(arg) && to_app(arg)->get_family_id() == m_bv_rw.get_fid()) {
                st = m_bv_rw.mk_bool_app(f, num, args, result);
                if (st != BR_FAILED) {
                  return st;
                }
              }
    ...
    br_status pull_ite(func_decl * f, unsigned num, expr * const * args, expr_ref & result) {
        ...
        if (num == 2 && (fid == m().get_basic_family_id() || fid == m_a_rw.get_fid() || fid == m_bv_rw.get_fid())) {
            ...
            if (m_bv_rw.is_mul(f)) {
              ...
              if (m().is_ite(args[1], a, b, c) && (m_bv_rw.is_zero(b) || m_bv_rw.is_zero(c))) {
                expr_ref mul_left(m().mk_app(f, args[0], b), m());
                expr_ref mul_right(m().mk_app(f, args[0], c), m());
                result = m().mk_ite(a, mul_left, mul_right);
                return BR_REWRITE2;
              }

            // (bvmul (ite a b c) u) --> (ite a (bvmul b u) (bvmul c u)) if b == 0 
              if (m().is_ite(args[0], a, b, c) && (m_bv_rw.is_zero(b) || m_bv_rw.is_zero(c))) {
                expr_ref mul_left(m().mk_app(f, b, args[1]), m());
                expr_ref mul_right(m().mk_app(f, c, args[1]), m());
                result = m().mk_ite(a, mul_left, mul_right);
                return BR_REWRITE2;
              }
            }
