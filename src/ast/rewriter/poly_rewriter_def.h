...
// result: true
template<typename Config>
br_status poly_rewriter<Config>::distrib_add_mul_eq(expr * lhs, expr * rhs, expr_ref & result) {
  if (! (is_mul(lhs) && to_app(lhs)->get_num_args() == 2
        && is_add(rhs))) {
    return BR_FAILED;
  }
  SASSERT(is_mul(lhs) && to_app(lhs)->get_num_args() == 2
      && is_add(rhs));

  expr *lhs_factor = nullptr;
  expr *lhs_add = nullptr;
  if (is_add(to_app(lhs)->get_arg(0))) {
    lhs_add = to_app(lhs)->get_arg(0);
    lhs_factor = to_app(lhs)->get_arg(1);
  }
  else if (is_add(to_app(lhs)->get_arg(1))) {
    lhs_add = to_app(lhs)->get_arg(1);
    lhs_factor = to_app(lhs)->get_arg(0);
  }
  else {
    return BR_FAILED;
  }
  ...
  unsigned lhs_sz;
  expr * const * lhs_monomials = get_monomials(lhs_add, lhs_sz);
  ...
  for (unsigned i = 0; i < lhs_sz; ++i) {
    expr *arg = lhs_monomials[i];
    SASSERT(!visited.is_marked(arg)); // we assume that the rewriter for adder will combine same args
    visited.mark(arg);
  }

  expr_fast_mark2 visited_test;
  for (unsigned i = 0; i < rhs_sz; ++i) {
    expr *mul = rhs_monomials[i];
    if (!is_mul(mul)) {
      return BR_FAILED;
    }

    expr_ref_buffer mul_args(m());
    unsigned sz = to_app(mul)->get_num_args();
    for (unsigned j = 0; j < sz; ++j) {
      expr *e = to_app(mul)->get_arg(j);
      if (e == lhs_factor) {
        continue;
      }
      mul_args.push_back(e);
    }
    if (mul_args.size() != sz - 1) { // no lhs_factor found
      return BR_FAILED;
    }

    expr_ref mul_other(mk_mul_app(mul_args.size(), mul_args.data()), m()); // it also handle size == 1

    if (!visited.is_marked(mul_other)) { // found find different arg
      return BR_FAILED;
    }

    // TODO remove this code if we are sure same args has been combined
    if (visited_test.is_marked(mul_other)) { // verify add rewrite will combine same arg
      SASSERT(false);
      return BR_FAILED;
    }
    else {
      visited_test.mark(mul_other);
    }
  }
